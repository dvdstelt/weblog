...
Note that at the bottom, I’ve filled in the update location. For some reason, when you deploy your application with auto update turned off, it won’t be able to find the update location anymore. It’s being removed from the manifest! I have no idea if this is a bug or not, but it can be solved by entering the update location in the bottom textbox. Now the [manual check for updates with ClickOnce](/2007/11/05/manual-check-for-updates-with-clickonce/) works again.
