...
Two more Duende-vs-Keycloak gotchas worth knowing.

**ServicePulse shows "User" instead of the username in the top-right.** ServicePulse reads `preferred_username` from the ID token for the display name. Keycloak emits that claim automatically from a user's `username` field. Duende's `TestUser` has a `Username` property used for login matching, but that property is not projected as a claim; if no test user carries an explicit `PreferredUserName` claim, the token doesn't have one and ServicePulse falls back to the literal "User". The sample's `Pages/TestUsers.cs` lists `JwtClaimTypes.PreferredUserName` on both `arya` and `jon` for that reason.

**`invalid_scope` for `offline_access`.** ServicePulse's `oidc-client-ts` always appends `offline_access` to the requested scopes (it's the standard OIDC scope for "issue me a refresh token too"). Keycloak silently accepts the request and decides at issuance time whether to actually mint a refresh; Duende rejects the request outright unless the client opts in via `AllowOfflineAccess = true`. The sample's `Config.cs` enables it for that reason: the server is then free to issue a refresh token, and ServicePulse decides whether to use it. If you removed that flag and now see `Sorry, there was an error : invalid_scope` on the Duende login page, that's the cause.

One more Duende-specific gotcha worth flagging: **signing-key regeneration on container restart**. Duende generates a new JWT signing key on startup unless you persist it. After a restart, any token already issued is unsigned by the new key set and gets rejected; the symptom is "I was logged in five minutes ago and now every XHR returns 401". Fix is to log in again. The permanent fix is the EF integration, which persists keys in the operational store; that's the same "what this leaves out" trade-off as in-memory configuration from Part 5.
